          ...
          xhr.onreadystatechange = function () {
            if (this.readyState == 4) {
              if (200 <= this.status && this.status <= 299) {
                ...
                cache
                  .put(cacheRequest, cacheResponse)
                  .then(function () {
                    resolve()
                  })
                  .catch(function (err) {
...
function _loadResources(cacheName, resourceFiles) {
  return new Promise(function (resolve, reject) {
    caches
      ...
      .then(function (cache) {
        Promise.all(
          Object.keys(resourceFiles).map(function (name) {
      ...
      .then(function (cachedNames) {
        Promise.all(
          cachedNames.map(function (cachedName) {
            if (
              cachedName === cacheNamePrefix + process.env.ND_DOCXWASM_VERSION ||
          ...
          .then(function () {
            Promise.all([
              ...
              .then(function (resources) {
                const ndResources = {
                  '@nativedocuments/docx-wasm': Object.keys(resources[0]).reduce(function (
                    ret,
                    name,
                  ) {
                  ...
                  '@nativedocuments/fontpool': function (filename) {
                    if (filename in this) {
                      ...
                      if (
                        ...
                        const msg =
                          'XMLHttpRequest(' + url + '): ' + this.status + ' ' + this.statusText
                        console.error(msg)
                        self.postMessage(['fetch', 'error', url, 0, msg])
                        return null
                      }
                    ...
                    Object.keys(resources[1]).reduce(function (ret, name) {
  ...
  if ('init' === msg.data[0]) {
    ...
    loadResources()
      ...
      .catch(function (err) {
        ...
        self.postMessage([
          'error',
          'resources initialization failed',
          JSON.stringify(err.toString()),
        ])
      })
  }
